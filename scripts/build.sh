...
CARTOOL_PATH="$BUILD_OUTPUT_DIR"/Products/Release/cartool
...
# been no file changes to the cartool code, then skip building.
#
# If we're being told to test, we should always build.
if [ "$BUILD_NEEDED" -eq 0 ]
then
  echo -n "Skipping build since product already exists at '$CARTOOL_PATH' and "
  echo    "no file changes have been made on top of the last commit."
  exit 0
fi

# xcodebuild intermittently crashes while building cartool.
#
...
BUILD_OUTPUT_PATH=$(/usr/bin/mktemp -t cartool-build)
...
while true; do
  ...
  if ! grep -q -E '(BUILD|CLEAN) (SUCCEEDED|FAILED)' "$BUILD_OUTPUT_PATH"; then
    ...
    if [[ $ATTEMPTS -le 10 ]]; then
      echo
      echo -n "xcodebuild appears to have crashed while building cartool; "
      echo    "retrying..."
      echo
      continue
    else
      echo
      echo -n "xcodebuild crashed while building cartool; giving up "
      echo    "after 10 tries."
      echo
      exit 1
    fi
